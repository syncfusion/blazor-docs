...
In the following examples, demonstrate how to enable **Localization** for DataGrid in server side Blazor samples. Here, we have used Resource file to translate the static text of the DataGrid.
...
namespace BlazorApplication
{
     public class SampleLocalizer : ISyncfusionStringLocalizer
    {
        ...
        public System.Resources.ResourceManager Manager
        {
            get
            {
                return DataGridBlazor.Resources.SyncfusionBlazorLocale.ResourceManager;
            }
        }
    }
}
...
* Finally, Specify the culture for DataGrid using [`locale`](https://help.syncfusion.com/cr/blazor/Syncfusion.Blazor~Syncfusion.Blazor.Grids.SfGrid%601~Locale.html) property.

```csharp
@using Syncfusion.Blazor.Grids

<SfGrid DataSource="@Orders" AllowPaging="true" AllowSorting="true" Locale="de" AllowGrouping="true">
...
The following example demonstrates the DataGrid in **Deutsch** culture. Here we use **LoadLocaleData** method to load the **locale.json** file and **SetCulture** method to set the culture of the DataGrid in Blazor WebAssembly sample.

For Blazor WebAssembly, we use **JSON** file which contains translated text of the culture. The following list of properties and its values are used in the DataGrid.
...
EmptyDataSourceError |DataSource must not be empty at initial load as columns are generated from the dataSource in AutoGenerate Column DataGrid.
Add | Add
Edit| Edit
Cancel| Cancel
Update| Update
...
The **Internationalization** library is used to globalize number, date, and time values in datagrid component using format strings in the **Format**. In the below sample we set the culture and currency using the **SetCulture** and **SetCurrencyCode** methods.
...
RTL provides an option to switch the text direction and layout of the DataGrid component from right to left. It improves the user experiences and accessibility for users who use right-to-left languages (Arabic, Farsi, Urdu, etc.). In the Below sample **EnableRtl** method is used to enable RTL in the DataGrid.
...
The locale.json file is as follows:
![Locale](./images/global-local.png)
